...
%                   Laplace equation
%
%    adaptiveAlgorithm computes the Q1-AFEM solution of the Laplace 
...
%          4) refine the finite element mesh (via QrefineR.m or QrefineR2.m 
%             - depending on the dimensions of the constrains input)
%          5) return to 1)
%
%Usage:
%
...
%    values of the Q1-AFEM solution as well the corresponding
...
while 1
  ...
  if size(constrains,2) == 3 
     ...
     indicators = computeEtaR2(x,coordinates,elements,constrains, ...
                           dirichlet,neumann,f,g);
  end                      
  data = [data;size(coordinates,1),sqrt(sum(indicators)),energy,H1s,H1,L2];
  %*** Stopping criterion
  ...
  if size(constrains,2) == 3 
      [coordinates,elements,constrains,dirichlet,neumann] ...
       = QrefineR(coordinates,elements,constrains,dirichlet,neumann,marked);
  else
      [coordinates,elements,constrains,dirichlet,neumann] ...
       =QrefineR2(coordinates,elements,constrains,dirichlet,neumann,marked);
  end
end
