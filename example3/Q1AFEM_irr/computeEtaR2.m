function etaR = computeEtaR2(x,coordinates,elements,constrains, ...
...
%
% *** try to estimate the error with values at the endpoints of interval
%Remark:
%
...
nT = size(elements,1);
[edge2nodes,foo{1},element2edges,dirichlet2edges,neumann2edges] ...
  =provideGeometricData(zeros(0,3),[elements;constrains],dirichlet,neumann);
...
if ~isempty(constrains)
  nI = size(element2edges,1) - nT;
  tmp = zeros(nI,4);
  
  ind = constrains(:,1) < constrains(:,2);
  tmp( ind,[1,4]) = etaR(element2edges(nT+find( ind),1),:)/3;
  tmp(~ind,[4,1]) = etaR(element2edges(nT+find(~ind),1),:)/3;
  tmp(:,[2,3]) = tmp(:,[1,4]) * [2,1;1,2]/3;
  
  ind = constrains(:,2) < constrains(:,3);
  tmp( ind,[4,3]) = tmp( ind,[4,3]) + etaR(element2edges(nT+find( ind),2),:);
  tmp(~ind,[3,4]) = tmp(~ind,[3,4]) + etaR(element2edges(nT+find(~ind),2),:);
  
  ind = constrains(:,4) < constrains(:,1);
  tmp( ind,[2,1]) = tmp( ind,[2,1]) + etaR(element2edges(nT+find( ind),4),:);
  tmp(~ind,[1,2]) = tmp(~ind,[1,2]) + etaR(element2edges(nT+find(~ind),4),:);

  etaR(element2edges(nT+1:end,1),:) = 0;
  etaR(element2edges(nT+1:end,2),:) = tmp(:,[3,4]);
  etaR(element2edges(nT+1:end,3),:) = tmp(:,[2,3]);
  etaR(element2edges(nT+1:end,4),:) = tmp(:,[1,2]);
end
...
%*** Incorporate constrains
if ~isempty(constrains)
  etaR(element2edges(nT+1:end,1)) = 3*sum(reshape(etaR(element2edges(nT+1:end,2:4)),[],3),2);
end
%etaR = sum(reshape(etaR(element2edges(1:nT,:)).^2,[],4),2);
